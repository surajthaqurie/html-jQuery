/*-- https://mmenujs.com/ : For Menu */
/* https://kenwheeler.github.io/slick/ : css(slick-theme.csss, slick.css, fonts),js(slick.min.js)-> For Slider (Slick Carousel jQuery) */
/* https://scrollrevealjs.org/ : ScrollReveal */
/* https://masonry.desandro.com/ : Image Gallery(GRID) */
/* https://jqueryvalidation.org/ : jQurey validator plugin */


var RockIt = {
    init: function () {
        // console.log('i rock you')
    ...
    homeAlbums: function () {
        $.getJSON("js/json/albums.json", function (data) {
            ...
            data.albums.forEach(element => {
                ...
                template += '<a href="' + element.url + '" targer="_blank">Get this record</a>';
                template += '</>';
    ...
    subscribeWidgetInputs: function () {
        ...
        function checkUserEmail(email) {
            $.ajax({
                url: "http://localhost:3030/email?email_like=" + email,
                type: "GET",
                dataType: "json",
                success: function (res) {
                    if (res.length <= 0) {
    ...
    masonryStartGallery: function (start, limit) {
        $.ajax({
            url: "http://localhost:3030/gallery?_start=" + start + "&_limit=" + limit,
            type: "GET",
    ...
    masonryAddBlock: function () {
        ...
        btn.click(function () {
            ...
            btn.hide();
            $.ajax({
                url: "http://localhost:3030/gallery?_start=" + start + "&_limit=" + RockIt.masonryLimit,
                type: "GET",
                success: function (response) {
    ...
    contactForm: function () {
        $('#contact_form').validate({
            rules: {
                contact_message: {               // name not (id or class)
                    required: true,
                    minlength: 20,
                },
                contact_name: {
